...
(function() {
  ...
  function visitDoc(yuiDoc, ternDef, options) {
    console.log('classItems: ', yuiDoc.classitems.length)
    console.log('classItems[0]: ', yuiDoc.classitems[0])
    console.log('classItems[1]: ', yuiDoc.classitems[1])
    // loop for classitems
  ...
  var p5moduleNames = ['Color', 'Shape', 'Constants',
                  'Structure', 'Environment', 'DOM',
                  'Rendering', 'Transform', 'Shape',
                  'Events', 'Image', 'IO',
                  'Math', 'Typography', 'Data',
                  'Lights, Camera', 'Sound']


  function visitClassItem(yuiClassItem, yuiDoc, ternDef, options) {
    var moduleName = yuiClassItem["module"]
    var className = yuiClassItem["class"]
    var name = yuiClassItem["name"]
    var isStatic = yuiClassItem["static"] === 1;
    var ternModule = getTernModule(moduleName, ternDef);
    var parent = null;

    if (className == 'p5') {
      // if (p5moduleNames.indexOf(moduleName) >= -1 ) {
      if (moduleName === 'yui') {
        parent = getTernClassOrPrototype(className, ternDef, yuiDoc, isStatic,
            options);
      } else {
        // This is how to get rid of "class: prototype: " nesting but it leaves the info from yuiDoc.classes undefined
        parent = ternModule;
      }
    } else {
    ...
    // !url
    ternClassItem["!url"] = getURL(options.baseURL, name);
  }
